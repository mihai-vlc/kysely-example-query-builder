...
const umzug = new Umzug({
        ...
        resolve: ({ name, path: filePath, context }) => ({
            ...
            down: async () => {
                ...
                if (!existsSync(downPath)) {
                    logger.info(
                        `No corresponding down migration found for ${name}`
                    );
                    return;
                }
    ...
    context: new SQLiteClient(dbDriver),
    logger: {
        info: (msg) => {
...
export async function run() {
    try {
        ...
        logger.error(
            `Failed to execute the database migrations: ${e}, ${e.stack}`
        );
    }
    return false;
}
